...
export class InstanceActionPromiseQueue {
  ...
  add(instanceAction: InstanceAction): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      ...
      if (!(this._activeItem != null && this._activeItem.instanceAction.type === instanceAction.type) && this._queue.find(item => item.instanceAction.type === instanceAction.type) == null) {
        this._queue.push({
          instanceAction: instanceAction,
          resolve: resolve,
          reject: reject
