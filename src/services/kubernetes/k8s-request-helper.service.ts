...
export class K8SRequestHelperService {
  ...
  validateHelper(image: Image, user: InstanceUser) {
    try {
      const volumes: IK8SRequestHelperVolumeData[] = this._requestHelper.getVolumes
        ? this._requestHelper.getVolumes(image, user) || []
        : [];
      const envVars: { name: string; value: string }[] = this._requestHelper.getEnvVars
        ? this._requestHelper.getEnvVars(image, user) || []
        : [];
      ...
      volumes.forEach(volume => {
        if (volume.name == null) {
          throw new Error(
            `K8SRequestHelper at '${APPLICATION_CONFIG().kubernetes.requestHelper}' must provides names for each volume`
          );
        }
        if (volume.volume == null) {
          throw new Error(
            `K8SRequestHelper at '${
              APPLICATION_CONFIG().kubernetes.requestHelper
            }' does not provide a 'volume' entity for volume '${volume.name}'`
          );
        }
        if (volume.volumeMount != null) {
          if (volume.volumeMount.mountPath == null) {
            throw new Error(
              `K8SRequestHelper at '${
                APPLICATION_CONFIG().kubernetes.requestHelper
              }' does not provide a 'volumeMount.mountPath' entity for volume '${volume.name}'`
            );
          }
          if (volume.volumeMount.readOnly != null && typeof volume.volumeMount.readOnly !== 'boolean') {
            throw new Error(
              `K8SRequestHelper at '${
                APPLICATION_CONFIG().kubernetes.requestHelper
              }' must does not provide a 'volumeMount.readOnly' that is of boolean type for volume '${volume.name}'`
            );
          }
        }
      });
      ...
      envVars.forEach(envVar => {
        if (envVar.name == null) {
          throw new Error(
            `K8SRequestHelper at '${
              APPLICATION_CONFIG().kubernetes.requestHelper
            }' must provides names for each environment variable`
          );
        }
        if (envVar.value == null) {
          throw new Error(
            `K8SRequestHelper at '${
              APPLICATION_CONFIG().kubernetes.requestHelper
            }' must provides values for each environment variable`
          );
        }
      });
      ...
      if (runAsUID != null && typeof runAsUID !== 'number') {
        throw new Error(
          `K8SRequestHelper at '${
            APPLICATION_CONFIG().kubernetes.requestHelper
          }' must provides run-as-UIDs that are numeric`
        );
      }
