...
describe('BaseRepository', () => {
  ...
  it('converts BETWEEN condition', async () => {
    ...
    expect(parameterizedClause.parameters.p1).to.equal(1);
    expect(parameterizedClause.parameters.p2).to.equal(2);
  });
  ...
  it('converts IN condition', async () => {
    ...
    expect(parameterizedClause.parameters.p1).to.equal(1);
    expect(parameterizedClause.parameters.p2).to.equal(2);
    expect(parameterizedClause.parameters.p3).to.equal(3);
  ...
  it('converts single AND condition', async () => {
    ...
    expect(parameterizedClause.parameters.p1).to.equal(1);
    expect(parameterizedClause.parameters.p2).to.equal('test');
  });
  ...
  it('converts combined AND condition', async () => {
    ...
    expect(parameterizedClause.clause).to.equal('x = :p1 AND y = :p2 AND a = :p3');
    expect(parameterizedClause.parameters.p1).to.equal(1);
    expect(parameterizedClause.parameters.p2).to.equal('test');
  ...
  it('converts combined AND and OR conditions with embeded >, < and =', async () => {
    ...
    expect(parameterizedClause.parameters.p4).to.equal(3);
    expect(parameterizedClause.parameters.p5).to.equal(2);
    expect(parameterizedClause.parameters.p6).to.equal(10);
  });
  ...
  it('converts mulitple AND and OR conditions with embeded >, < and =', async () => {
    ...
    expect(parameterizedClause.parameters.p2).to.equal(3);
    expect(parameterizedClause.parameters.p3).to.equal(10);
    expect(parameterizedClause.parameters.p4).to.equal(1);
    expect(parameterizedClause.parameters.p5).to.equal(3);
    expect(parameterizedClause.parameters.p6).to.equal(2);
    expect(parameterizedClause.parameters.p7).to.equal(2);
    expect(parameterizedClause.parameters.p8).to.equal(3);
    expect(parameterizedClause.parameters.p9).to.equal(4);
